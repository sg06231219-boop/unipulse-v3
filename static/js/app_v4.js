...
(function() {
  ...
  function renderScoresTab(scores, u) {
    if (!scores || !scores.scores || Object.keys(scores.scores).length === 0) {
      return '<p style="color:var(--gray-400)">暂无分数线数据</p>';
    }
    const provScores = scores.scores;
    let html = '<table class="score-table"><thead><tr><th>省份</th><th>科类</th><th>批次</th><th>最低分</th><th>省控线</th><th>线差</th><th>最低位次</th></tr></thead><tbody>';
    for (const [prov, items] of Object.entries(provScores)) {
      if (Array.isArray(items)) {
        items.forEach(s => {
          html += `<tr>
            <td>${esc(s.province || prov)}</td>
            <td>${esc(s.type || '')}</td>
            <td>${esc(s.batch || '')}</td>
            <td><strong>${s.min_score || '-'}</strong></td>
            <td>${s.proscore || '-'}</td>
            <td style="color:${(s.diff||0) > 100 ? 'var(--red)' : 'var(--primary)'}">${s.diff ? '+' + s.diff : '-'}</td>
            <td>${s.min_rank || '-'}</td>
          </tr>`;
        });
      }
