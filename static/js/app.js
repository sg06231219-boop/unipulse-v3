...
function getLevelClass(level) {
  ...
  if (level.includes('双一�?)) return 'level-dy';
  return 'level-other';
}
function getChanceInfo(gap) {
  if (gap >= 30) return {text:'稳上', cls:'chance-bao', group:'�?};
  if (gap >= 20) return {text:'较稳', cls:'chance-bao', group:'�?};
  if (gap >= 10) return {text:'有把�?, cls:'chance-wen', group:'�?};
  if (gap >= 0) return {text:'可冲', cls:'chance-wen', group:'�?};
  if (gap >= -10) return {text:'有风�?, cls:'chance-chong', group:'�?};
  if (gap >= -20) return {text:'较难', cls:'chance-chong', group:'�?};
  if (gap >= -30) return {text:'困难', cls:'chance-none', group:'�?};
  return {text:'差距�?, cls:'chance-none', group:''};
}
function formatSalary(v) { return v >= 10000 ? (v/1000).toFixed(0)+'K' : v; }
function tagType(t) {
  const map = {'985':'gold','211':'blue','双一�?:'cyan','985/211':'gold','独立学院':'orange','省属重点':'green','外语顶尖':'purple','传媒顶尖':'red','法学顶尖':'gold','药学顶尖':'red','电子信息':'cyan','两电一�?:'blue','外交官摇�?:'gold','经贸顶尖':'gold','政法黄埔':'red','外语':'purple','师范':'cyan','医学':'red','财经':'purple','建筑':'orange','农林':'green','理工':'blue','综合':'cyan'};
...
async function loadHeroStats() {
  try {
    ...
    $('heroStats').innerHTML = `
      ...
      <div class="stat-pill">💼 <strong>${s.employment_records}</strong>条就业数�?/div>
      <div class="stat-pill">💰 平均起薪 <strong>${(s.avg_salary/1000).toFixed(0)}K</strong></div>
      <div class="stat-pill">📊 平均就业�?<strong>${s.avg_employment_rate}%</strong></div>
...
async function loadScoreDistribution() {
  try {
    ...
    $('scoreDistChart').innerHTML = data.map(d => {
      ...
      return `<div class="score-dist-bar" onclick="navigate('universities');$('scoreSlider').value=${parseInt(d.range.split('-')[0])};$('scoreDisplay').textContent='${d.range.split('-')[0]}�?;filterByScore(${parseInt(d.range.split('-')[0])})">
...
function renderUniCard(u, showChance = false) {
  ...
  return `<div class="uni-card" data-page="uni-detail" data-id="${u.id}">
    ...
    ${chance ? `<div class="uni-card-chance ${chance.cls}">${userScore}�?· ${chance.text}（差${Math.abs(gap)}分）</div>` : ''}
    <div class="uni-card-stats">
      <span class="uni-stat">就业�?<strong>${u.employment_rate}%</strong></span>
    ...
    <div class="uni-card-actions">
      <button class="btn btn-xs ${isInWish?'btn-primary':'btn-ghost'}" onclick="event.stopPropagation();addToWish(${u.id},'${esc(u.name)}',${u.gaokao_score})">${isInWish?'�?已加志愿':'+志愿�?}</button>
...
async function loadUniversities(page = 1) {
  ...
  try {
    ...
    $('uniResultsInfo').textContent = `�?${chanceFilter && userScore ? data.length : r.total} 所高校`;
    ...
    <div class="uni-detail">
      ...
      <div class="uni-detail-header">
        <div class="uni-detail-info">
          ...
          ${chance ? `<div style="margin-top:1rem"><span class="uni-card-chance ${chance.cls}" style="font-size:0.9rem;padding:6px 16px">${userScore}�?· ${chance.text}（差${Math.abs(gap)}分）</span></div>` : ''}
        </div>
      ...
      <div class="uni-detail-metrics">
        ...
        <div class="metric-card">
          ...
          <div class="metric-label">平均起薪/�?/div>
        </div>
        <div class="metric-card">
          <div class="metric-val" style="color:var(--yellow)">¥${u.tuition?.toLocaleString()}/�?/div>
      ...
      <div style="overflow-x:auto">
        <table class="emp-table">
          <thead><tr><th>专业</th><th>平均薪资</th><th>起薪</th><th>就业�?/th><th>内卷指数</th><th>前景</th></tr></thead>
      ...
      <div class="detail-tabs" style="margin-top:1.5rem">
        ...
        <button class="detail-tab" onclick="switchDetailTab('province',this)">🗺�?省分数线</button>
...
async function loadProvinceMajorScores(uniId, containerId) {
  ...
  try {
    ...
    html += '<option value="name">按省�?/option><option value="score-asc">分数低→�?/option><option value="score-desc">分数高→�?/option>';
    if (userScore) html += '<option value="gap-desc">差距大→�?/option>';
    html += '</select>';
    // 统计
    html += '<span id="provCount" style="color:var(--text3);font-size:0.82rem;margin-left:auto">' + provinces.length + '个省�?· ' + Object.values(majorScores).reduce((a, b) => a + b.length, 0) + '条专业分数线</span>';
    html += '</div>';

    // ── 用户分数提示 ──
    if (userScore) {
      html += '<p style="color:var(--text2);margin-bottom:0.6rem;font-size:0.85rem">💡 以你�?<strong>' + userScore + '�?/strong> 为基准，点击省份展开专业分数�?/p>';
    } else {
      html += '<p style="color:var(--text2);margin-bottom:0.6rem;font-size:0.85rem">💡 在首页输入你的分数，可查看各省份录取概率 | 点击省份展开专业分数�?/p>';
    }

    // ── 省份表格 ──
    html += '<div style="overflow-x:auto"><table class="emp-table" id="provTable"><thead><tr>';
    html += '<th style="width:28px"></th><th>省份</th><th>校线</th><th>专业�?/th><th>年份</th>';
    ...
    for (const prov of provinces) {
      ...
      if (hasMajors) {
        ...
        html += '<table class="emp-table prov-major-table" style="margin:0;font-size:0.82rem"><thead><tr><th>专业名称</th><th>科类</th><th>选科要求</th><th>最低分</th><th>最低位�?/th><th>年份</th>';
        if (userScore) html += '<th>差距</th><th>评估</th>';
        html += '</tr></thead><tbody>';
        for (const m of majors) {
          ...
          // 冲稳保分色：> score+20=�?�?, > score=�?�?, else=�?�?
          let chanceLabel, chanceColor, chanceGroup;
          if (mGap !== null) {
            if (mGap > 20) { chanceLabel = '�?; chanceColor = 'var(--accent2)'; chanceGroup = 'wen'; }
            else if (mGap > 0) { chanceLabel = '�?; chanceColor = 'var(--red)'; chanceGroup = 'chong'; }
            else { chanceLabel = '�?; chanceColor = 'var(--green)'; chanceGroup = 'bao'; }
          }
          html += '<tr class="major-row" data-type="' + esc(mType) + '">';
          html += '<td style="white-space:nowrap">' + esc(mName) + '</td>';
          html += '<td><span class="tag tag-' + (mType.includes('�?) ? 'accent2' : mType.includes('�?) ? 'warning' : 'info') + '" style="font-size:0.72rem;padding:1px 5px">' + esc(mType) + '</span></td>';
...
function filterProvRows() {
  ...
  if (countEl) countEl.textContent = visible + '个省�?;
}
  ...
  $('compareSlots').innerHTML = Array.from({length:5}, (_, i) => {
    ...
    if (item) {
      return `<div class="compare-slot filled">
        ...
        <div class="slot-score">${item.score}�?/div>
      </div>`;
    }
...
async function pickForCompare(slot) {
  ...
  try {
    ...
    if (r.data.length === 0) { toast('未找�?); return; }
...
function addToCompare(id, name, score) {
  if (compareList.length >= 5) { toast('最多对�?所'); return; }
  if (compareList.some(c => c.id === id)) { toast('已在对比�?); return; }
...
async function loadProgramDetail(name) {
  try {
    ...
    $('programDetailContent').innerHTML = `
      ...
      <p style="color:var(--text2);margin:0.5rem 0 1rem">�?${unis.length} 所高校开设此专业</p>
...
async function toggleBookmark(postId) {
  try {
    ...
    if (btn) btn.textContent = bookmarkedPosts.has(postId) ? '�?已收�? : '�?收藏';
  } catch(e) { toast('操作失败'); }
}
    ...
    $('postDetailContent').innerHTML = `
      <div class="post-detail">
        ...
        <div class="post-detail-actions">
          ...
          <button class="btn btn-sm btn-ghost bookmark-btn" data-id="${id}" onclick="toggleBookmark(${id})">${bookmarked?'�?已收�?:'�?收藏'}</button>
          ${p.can_edit ? `<button class="btn btn-sm btn-ghost" onclick="editPost(${id})">✏️ 编辑</button>` : ''}
          ${p.can_edit ? `<button class="btn btn-sm btn-ghost" onclick="deletePost(${id})" style="color:var(--red)">🗑�?删除</button>` : ''}
...
async function submitAiReport(e) {
  ...
  if (!score || score < 300 || score > 750) { toast('请输入有效分�?300-750)'); return; }
  userScore = score;
  localStorage.setItem('unipulse_score', score);
  // 付费墙：免费3次，之后9.9元解�?
  if (!Paywall.tryUse('ai-report', { price: '9.9', qrImg: '/static/img/donate-qr.png', desc: 'AI选校报告 & 冲稳保推�?& 录取概率分析 & 分数线对�?, freeLimit: 3, contactWx: 'a5050e' })) {
...
function renderAiReport(r) {
  const groups = [
    {key:'�?, label:'🎯 冲一�?, desc:'分数略高于你的院校，有录取希�?, cls:'gap-chong', color:'var(--red)'},
    {key:'�?, label:'�?稳一�?, desc:'分数与你相当，录取概率较�?, cls:'gap-wen', color:'var(--green)'},
    {key:'�?, label:'🛡�?保一�?, desc:'分数低于你的院校，确保不滑档', cls:'gap-bao', color:'var(--accent2)'},
  ];
  $('aiReportResult').innerHTML = `
    <div style="text-align:center;margin-bottom:1.5rem;padding:1.5rem;background:var(--surface);border-radius:var(--radius);border:1px solid var(--border)">
      <div style="font-size:2.5rem;font-weight:900;color:var(--accent2)">${r.score}�?/div>
      <div style="color:var(--text3)">${r.province} · ${r.preference==='综合'?'综合平衡':r.preference==='就业'?'优先就业':r.preference==='深�??'优先深�?:'优先城市'}</div>
    </div>
    ${groups.map(g => {
      ...
      return `<div class="report-section">
        ...
        <div class="report-group">${list.map(u => {
          ...
          return `<div class="report-card" data-page="uni-detail" data-id="${u.id}">
            ...
            <div class="rc-score">${u.gaokao_score}�?· ${u.loc} · ${u.level.split('/')[0]}</div>
            <span class="rc-gap ${g.cls}">${gap>0?'�?+gap+'�?:'�?+Math.abs(gap)+'�?}</span>
          </div>`;
        }).join('')}</div>
      </div>`;
    }).join('')}
...
async function loadFavorites() {
  try {
    ...
    if (list.length === 0) {
      $('favGrid').innerHTML = `<div class="empty-state"><div class="empty-icon">�?/div><h3>暂无收藏</h3><p>浏览高校时点击⭐收藏</p><button class="btn btn-primary" data-page="universities">去浏�?/button></div>`;
    } else {
...
async function performSearch(q) {
  ...
  try {
    ...
    if (!html) html = '<div class="empty-state"><h3>未找到结�?/h3></div>';
...
function renderPagination(containerId, total, limit, current, onClick) {
  ...
  let html = '';
  if (current > 1) html += `<button onclick="(${onClick})(${current-1})">上一�?/button>`;
  const start = Math.max(1, current - 2);
  const end = Math.min(pages, current + 2);
  for (let i = start; i <= end; i++) {
    html += `<button class="${i===current?'active':''}" onclick="(${onClick})(${i})">${i}</button>`;
  }
  if (current < pages) html += `<button onclick="(${onClick})(${current+1})">下一�?/button>`;
...
function updateWishBadge() {
  ...
  if (nav) {
    ...
    nav.textContent = cnt > 0 ? `📋 志愿�?${cnt})` : '📋 志愿�?;
  }
}
...
async function loadWishTable() {
  ...
  try {
    ...
    if (serverData.�?.length || serverData.�?.length || serverData.�?.length) {
      wishList = [];
      ['�?,'�?,'�?].forEach(g => {
  ...
  $('wishChongCount').textContent = groups['�?].length;
  $('wishWenCount').textContent = groups['�?].length;
  $('wishBaoCount').textContent = groups['�?].length;
  ...
  // Render groups
  ['�?,'�?,'�?].forEach(g => {
    const container = $('wishItems' + (g==='�??'Chong':g==='�??'Wen':'Bao'));
    if (!container) return;
    container.innerHTML = groups[g].map((w, i) => `
      <div class="wish-item" draggable="true" data-wish-id="${w.id}" data-wish-group="${g}" data-wish-idx="${i}">
        ...
        <div class="wish-item-info">
          ...
          <div class="wish-item-meta">${w.score}�?· ${g}�?/div>
        </div>
        <div class="wish-item-btns">
          ${g!=='�??'<button class="btn btn-xs btn-ghost" onclick="moveWishGroup('+w.id+',\'冲\')" title="移到冲组">🎯</button>':''}
          ${g!=='�??'<button class="btn btn-xs btn-ghost" onclick="moveWishGroup('+w.id+',\'稳\')" title="移到稳组">�?/button>':''}
          ${g!=='�??'<button class="btn btn-xs btn-ghost" onclick="moveWishGroup('+w.id+',\'保\')" title="移到保组">🛡�?/button>':''}
          <button class="btn btn-xs btn-ghost" onclick="navigate('uni-detail',{id:${w.id}})" title="查看详情">👁�?/button>
          <button class="btn btn-xs btn-ghost" style="color:var(--red)" onclick="removeFromWish(${w.id})" title="移除">�?/button>
        </div>
      </div>`).join('') || '<div class="wish-empty-group">点击高校卡片上的 +志愿�?添加</div>';
...
function renderWishListView(groups) {
  ...
  table.innerHTML = `<table class="emp-table">
    <thead><tr><th>序号</th><th>分组</th><th>院校</th><th>参考线</th><th>与我的差�?/th><th>操作</th></tr></thead>
    <tbody>${all.map((w,i) => {
      const gap = userScore ? userScore - w.score : null;
      return `<tr>
        <td>${i+1}</td>
        <td><span class="uni-card-chance ${w.group==='�??'chance-chong':w.group==='�??'chance-wen':'chance-bao'}">${w.group}</span></td>
        <td><strong>${esc(w.name)}</strong></td>
        <td>${w.score}�?/td>
        <td>${gap!==null?(gap>=0?'+':'')+gap+'�?:'-'}</td>
...
async function submitNewPost() {
  ...
  if (!title) { toast('请输入标�?); return; }
  if (!content) { toast('请输入内�?); return; }
...
function drawCompareRadar(canvasId, unis) {
  ...
  const dims = ['综合实力', '学科建设', '就业质量', '师资力量', '国际�?, '科研水平'];
  const n = dims.length;
